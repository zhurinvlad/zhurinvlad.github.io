...
# To convert a Jekyll blog post file name to a zhurinvlad.com URL, we need to:
# 
# 1. Strip folder prefix
# 2. Strip markdown file extension
# 3. Replace the dashes in the date with slashes
# 4. Prepend zhurinvlad.com
#
# Example:
#
...
conditionals = files.map do |file|
  ...
  if first_match
    ...
    {:conditional => conditional, :zhurinvlad_url => zhurinvlad_url}
  end
end
...
if_statement_body = REDIRECT_TEMPLATE.gsub("__REDIRECT_URL__", first_conditional[:zhurinvlad_url])
...
elseif_statements = conditionals[1..-1].map do |conditional|
  ...
  elseif_statement_body = REDIRECT_TEMPLATE.gsub("__REDIRECT_URL__", conditional[:zhurinvlad_url])

  "#{elseif_statement}\n#{elseif_statement_body}"
end

else_statement_body = REDIRECT_TEMPLATE.gsub("__REDIRECT_URL__", zhurinvlad_DOMAIN)
